...
In addition, VUAs can query an index (built over the VPR Participant registry - see below) that catalogs all known verifiable services (VSs), to search VSs compatible with the VUA or VSs that present a certain type of credential. This enables:
...
It is used by ecosystems — each controlled by a **Corporation** — that want to define credential schemas and who can issue or verify credentials of these schemas.
...
[Ecosystems are detailed here](../verifiable-public-registry/ecosystems).
