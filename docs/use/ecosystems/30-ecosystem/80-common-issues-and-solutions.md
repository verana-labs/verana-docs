...
- **`operator authorization not found for this corporation/operator pair`** — the account passed to `--from` has not been granted authorization for this message type on behalf of the `[corporation]`. Grant it via the [Delegation module](../../corporation/delegation/grant-operator-authorization), allow-listing the exact `Msg` type-URL (e.g. `/verana.ec.v1.MsgCreateEcosystem`).
- **`corporation not registered`** — the address passed as `[corporation]` is not a registered Corporation `policy_address`. Create one first (see [Create a Corporation](../../corporation/create-a-corporation)).
